...
import { LoginErrorType } from '../types/auth.types';
...
export default function Login() {
  ...
  const validateForm = () => {
    ...
    if (typeof password !== "string" || RegexExpressions.STRONGER_PASSWORD_REGEX.test(password)===false) {
      setError({...error,password:[...error.password,"password must contain atleast one of a lowercase letter,an uppercase letter,a number and a special character."]});
      return false;
    }
    return true;
  };
        ...
        <div className="backdrop-blur-lg bg-white/10 rounded-2xl shadow-xl border border-white/20 p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              />
              {error.email && error.password.length>0 &&
                            <p className="text-red-500 text-sm">{error.email.join()}</p>
